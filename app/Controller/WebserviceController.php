...
class WebserviceController extends AppController {
    ...
    function beforeFilter() {
        ...
        $this->Auth->allow(array('get_countries', 'get_categories', 'getPageInfo', 'getip', 'sendApplyInfo', 'get_languages', 'get_slides', 'get_slides_app', 'get_sub_categories', 'get_products', 'get_modifiers', 'get_options', 'get_suboptions', 'getImagePath', 'get_all_categories_data', 'get_all_categories_data_app', 'getItemData', 'getItemDataApp', 'placeOrder', 'getStoreList', 'getStoresFromPostalCode', 'getStoresFromLatLong', 'getStoreDetails', 'login', 'getTwitterFeeds', 'getInstagramPost', 'getCountryStores', 'saveFavItem', 'getCitiesSuggestion', 'getCitiesSuggestionApp', 'getFBFeed', 'getIGFeed', 'getPrefrences', 'signUp', 'getFav', 'getFavItemData', 'applyCoupon', 'getFavOrderData', 'getProfile', 'sendCateringInfo', 'sendContactInfo', 'sendCareerInfo', 'getOrderHistory', 'updateProfile', 'getProductNameByPlu', 'getModifierName', 'updatePrefrence', 'addAddress', 'deleteAddress', 'editAddress', 'setAsDefault', 'getUserPrefreces', 'getAreaSuggestion', 'testUrl', 'getStoreDetailsByStoreId', 'forgot_password', 'reset_password', 'getReOrderData', 'sendAckEmail', 'uploadAttachment', 'sendPaymentData', 'getCountryCodes', 'doLogEntry', 'getDealItemList', 'getDealIdFromCode', 'getSuggestedProductsApp', 'get_all_categories_data_fullmenu', 'getAllStores', 'upComingStores'));
    }
    ...
    public function get_all_categories_data_app($storeId = 1, $menuCountry = 'UAE') {
        ...
        //Configure::write('debug', 2);
        $return = Cache::read('full_data_uae_app','api_cache');
        if ($return !== false) {
        ...
        //die;
        //echo '<pre>'; print_r($cats); die;
        echo json_encode(array('itemList' => $cats));
        Cache::write('full_data_uae_app', array('itemList' => $cats), 'api_cache');
        die;
    }

    public function getItemData($slug = '', $menuCountry = 'UAE') {
        //Configure::write('debug', 2);
        $key=md5('__'.$slug.$menuCountry);
        $return=Cache::read($key,'api_cache');
        if($return !== false){
            echo json_encode($return);
            die;
        }
        if ($slug != '') {
            ...
            Cache::write($key, $item, 'api_cache');
            die;
        }
    }

    public function getItemDataApp($slug = '', $menuCountry = 'UAE') {
        $key=md5('__App_'.$slug.$menuCountry);
        $return=Cache::read($key,'api_cache');
        if($return !== false){
            echo json_encode($return);
            die;
    ...
    public function getFormattedItemDataApp($slug, $menuCountry = 'UAE') {
        $this->Product->recursive = 6;
                        ...
                        foreach ($plu_json['modifier'] as $plu_mod) {
                            foreach ($plu_mod as $pl_m) {
                                ...
                                if ($pl_m['PLU'] == $mo['Option']['plu_code']) {
                                    ...
                                    $item['ProductModifier'][$i]['Modifier']['ModifierOption'][$j]['Option']['price'] = ($price != '0.00') ? $price : null;
                                    $item['ProductModifier'][$i]['Modifier']['ModifierOption'][$j]['Option']['is_topping'] = true;
                                    //$mo['price'] = $pl_m['Price'];
                                }
                            }
        ...
        if (!empty($data['order_details'])) {
            ...
            foreach ($data['order_details'] as $ord) {
                ...
                if ($ord['category_id'] == 1) {
                    ...
                    if (isset($ord['dealId'])) {
                        $pizzaArr['dealId'] = $ord['dealId'];
                    }

                    if (isset($ord['comboUniqueId'])) {
                        $pizzaArr['comboUniqueId'] = $ord['comboUniqueId'];
                    }
    ...
    function sendCateringInfo() {
        ...
        if (!empty($data)) {
            ...
            if ($this->sendPhpEmail(CATERING_EMAIL, $from, $subject, $content)) {
                $this->sendAckEmail($email, $username, 'catering_notification_reply');
    ...
    function sendApplyInfo() {
        ...
        if (!empty($data)) {
            ...
            if ($this->sendPhpEmail(ENQUIRY_EMAIL, $from, $subject, $content)) {
                $this->sendAckEmail($email, $name, 'franchise_enquiry_reply');
    ...
    function sendCareerInfo() {
        ...
        if (!empty($data)) {
            ...
            if ($this->sendPhpEmail(CAREER_EMAIL, $from, $subject, $content)) {
                $this->sendAckEmail($email, $name, 'career_notification_reply');
                echo json_encode(array('show' => true, 'isSuccess' => true, 'message' => 'Thank You ! information has been sent successfully, will contact you soon.'));
            } else {
                echo json_encode(array('show' => true, 'isSuccess' => false, 'message' => 'Sorry ! mail not send, please try again.'));
            }
    ...
    public function testUrl() {
        ...
        die;
        echo 'https://' . $_SERVER['SERVER_NAME'] . '/beta';
        die;
    }
    ...
    function sendAckEmail($emailId, $username, $template = 'user_acknowledgement') {
    ...
    public function sendPaymentData() {
        $paymentDetails = $this->request->input('json_decode', true);

        if (!empty($paymentDetails)) {
            $paymentDetails['order_details'] = $this->formatPlaceOrderData($paymentDetails);
            //$details = json_encode($details);
            //echo $details; die;
            $url = APIURL . '/index.php/Pay';
            //$url = 'http://35.185.240.172/nkd/index.php/Pay';

            $result = $this->curlPostRequest($url, $paymentDetails);
            //$result     = $this->curlGetRequest($url);
            $response = json_decode($result);
    ...
    public function getCountryCodes() {
        ...
        $code = $this->Countrycode->find('all');
        echo json_encode($code);
        die;
    }

    public function doLogEntry() {
        Configure::write('debug', 2);
        $logData = $this->request->input('json_decode', true);
        if (!empty($logData)) {

            $data = array(
                'unique_id' => $logData['orderLogId'],
                'step' => $logData['step'],
    ...
    function getDealItemList($id = '') {
        ...
        foreach ($deals as $key => $value) {
            ...
            $itemArr = $this->getFilterDealCondition($itemArr);
    ...
    private function getFilterDealCondition($dealCategories) {
        for ($i = 0; $i < count($dealCategories); $i++) {
            $dealCategories[$i]['catList'][] = $dealCategories[$i]['id'];
            //$dealCategories[$i]['pos']=$i;
            if ($dealCategories[$i]['itemCondition'] == 'OR') {
                $dealCategories[$i]['catList'][] = $dealCategories[$i + 1]['id'];
                $prod = !empty($dealCategories[$i]['products']) ? $dealCategories[$i]['products'] : array();
                $prod1 = !empty($dealCategories[$i + 1]['products']) ? $dealCategories[$i + 1]['products'] : array();
                $prod = array_merge($prod, $prod1);
    ...
    function getSuggestedProductsApp($storeId = 1) {
    ...
    public function getAllStores() {
        $storesUkUrl = 'https://nkdpizza.com/uk/dev/webservice/getStores';
        $stores = $this->Store->find('all', array(
            'conditions' => array(
                'Store.status' => 1
            )
        ));
        $curlUk = curl_init($storesUkUrl);
        curl_setopt($curlUk, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curlUk, CURLOPT_FOLLOWLOCATION, 1);
        curl_setopt($curlUk, CURLOPT_SSL_VERIFYPEER, 0);
        curl_setopt($curlUk, CURLOPT_SSL_VERIFYHOST, 0);
        $ukData = curl_exec($curlUk);
        $ukData = json_decode($ukData);
        $data = array_merge($stores, $ukData);
        echo json_encode($data);
        die;
    }

    public function upComingStores() {
        //$country = strtolower($country);
        $stores = $this->Store->find('all', array(
            'conditions' => array(
                'Store.store_status' => 'Upcoming',
            )
        ));
        echo json_encode($stores);
        die;
    ...
    public function get_all_categories_data_fullmenu($storeId = 1, $menuCountry = 'UAE') {
        $return = Cache::read('full_data_uae','api_cache');
        if ($return !== false) {
            echo json_encode($return);
            die;
        }
        ...
        $this->Category->bindModel(array('hasMany' => array(
                'Product' => array(
                    ...
                    'fields' => array(
                        ...
                        'Product.sub_category_id', 'Product.short_description', 'Product.plu_code',
                        'Product.title', 'Product.price_title', 'Product.slug', 'Product.price', 'Product.image',
                        'Product.thumb_image', 'Product.sort_order', 'groupv', 'grouph', 'groupvh', 'dd_default_selection', 'dd_group_id', 'groupp'
                    ),
                    'order' => array('Product.sort_order' => 'asc')
                )
        )));
                            ...
                            if (!empty($plu_json)) {
                                foreach ($plu_json as $pluData) {
                                    ...
                                    foreach ($pluData as $key => $pdat) {
                                        ...
                                        if ($dat['Category']['id'] == '1') {
                                            $tmp_crust = null;
                                            if (isset($prod['dd_group_id']) && !empty($prod['dd_group_id'])) {
                                                $dropDown_PluCode = $this->getPluCodeByGroupID($prod['dd_group_id']);
                                                $tmp_crust = $dropDown_PluCode ? $this->getModifierForDD(array($crustData), $dropDown_PluCode) : array();
                                                $prod['crust_price'] = empty($tmp_crust) ? $crustData : $tmp_crust;
                                                //unset($prod['dd_group_id']);
                                            }
                                            $prod['crust_price'] = isset($tmp_crust) && !empty($tmp_crust) ? $tmp_crust : $crustData;
                                            $prod['dd_default_selection'] = $this->defaultDDSelection($prod['crust_price']);
        ...
        //echo '<pre>'; print_r($cats); die;
        echo json_encode($cats);
        Cache::write('full_data_uae', $cats, 'api_cache');
        die;
    }
        ...
        if (is_array($Modifier) && is_array($plu_code)) {
            foreach ($plu_code as $plu) {
                foreach ($Modifier as $mod) {
                    foreach ($mod as $mod1) {
                        if (isset($mod1['PLU']) && $mod1['PLU'] == $plu)
                            $return[] = $mod1;
                    }
                }
            }
    ...
    /**
     * 
     * @param type $crustonly for pizza
     */
    public function defaultDDSelection($crusts) {
        $return = false;
        if (is_array($crusts)) {
            ...
            $return = (empty($return) && isset($crusts[0]['PLU'])) ? $crusts[0]['PLU'] . '-999992' : $return;
        }
        return $return;
    }
